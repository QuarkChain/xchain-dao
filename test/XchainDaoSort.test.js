...
describe("XChainDaoSort", () => {
  let dao;

  beforeEach(async () => {
    const XChainDao = await ethers.getContractFactory("XchainDao");
    dao = await XChainDao.deploy();
    await dao.deployed();
  });

  it("setMaxSize(): should update the max size", async () => {
    const setMaxSizeTx = await dao.setMaxSize(42);
    await setMaxSizeTx.wait();
    expect(await dao.getMaxSize()).to.equal(42);
  });
